...
static void check_umlattr(umlattribute* u, char* typename) {
...
static char* subst(char* str, char search, char replace) {
  char* p;
...
static char* nospc(char* str) {
  return subst(str, ' ', '_');
}

static int pass_by_reference(umlclass* cl) {
  char* st;
  if (cl == NULL)
    return 0;
  st = cl->stereotype;
  ...
  return (!is_const_stereo(st) && !is_enum_stereo(st));
}

static int is_oo_class(umlclass* cl) {
  char* st;
  if (cl == NULL)
    return 0;
  st = cl->stereotype;
  if (strlen(st) == 0)
    return 1;
  return (!is_const_stereo(st) && !is_typedef_stereo(st) &&
          !is_enum_stereo(st) && !is_struct_stereo(st) &&
          !eq(st, "CORBAUnion") && !eq(st, "CORBAException"));
}

static int has_oo_class(declaration* d) {
  while (d != NULL) {
...
static char* cppname(char* name) {
  static char buf[SMALL_BUFFER];
  if (use_corba) {
    if (eq(name, "boolean") || eq(name, "char") || eq(name, "octet") ||
        eq(name, "short") || eq(name, "long") || eq(name, "float") ||
        eq(name, "double") || eq(name, "string") || eq(name, "any")) {
...
static char* fqname(umlclassnode* node, int use_ref_type) {
...
static void check_visibility(int* curr_vis, int new_vis) {
  if (*curr_vis == new_vis)
    return;
  indentlevel--;
  switch (new_vis) {
...
static void add_setter_getter(umlclassnode* node,
                              char* typename,
                              char* varname) {
  int foundGetter = 0;
  int foundSetter = 0;
...
static void gen_class(umlclassnode* node) {
  ...
  if (node->key->operations != NULL) {
    ...
    while (umlo != NULL) {
      ...
      if (strlen(umlo->key.attr.comment)) {
        ...
        while (tmpa != NULL) {
          print("/// @param %s\t\t(%s) %s\n", tmpa->key.name,
                kind_str(tmpa->key.kind), tmpa->key.comment);
          tmpa = tmpa->next;
        }
      }
      /* print operation */
...
static void gen_decl(declaration* d) {
  char* name;
  char* stype;
  umlclassnode* node;
  umlattrlist umla;
  ...
  if (eq(stype, "CORBANative")) {
    ...
    if (umla == NULL) {
      fprintf(stderr,
              "Error: first attribute (impl type) not set "
              "at typedef %s\n",
              name);
      exit(1);
    }
    if (strlen(umla->key.name) > 0) {
      fprintf(stderr,
              "Warning: typedef %s: ignoring name field "
...
void print_include_stdlib(struct stdlib_includes* si, char* name) {
  if (strlen(name) > 0) {
    if (!si->stdint && (strstr(name, "int8_t") || strstr(name, "uint8_t") ||
                        strstr(name, "int16_t") || strstr(name, "uint16_t") ||
                        strstr(name, "int32_t") || strstr(name, "uint32_t") ||
                        strstr(name, "int64_t") || strstr(name, "uint64_t"))) {
    ...
    if (!si->memory &&
        (strstr(name, "std::queue") || strstr(name, "std::priority_queue"))) {
      print("#include <queue>\n");
      si->queue = 1;
    }
    ...
    if (!si->memory &&
        (strstr(name, "std::unique_ptr") || strstr(name, "std::shared_ptr") ||
         strstr(name, "std::weak_ptr"))) {
      print("#include <memory>\n");
      si->memory = 1;
    }
    if (!si->random &&
        (strstr(name, "std::mt19937") || strstr(name, "std::random_device") ||
         strstr(name, "std::uniform_int_distribution"))) {
      print("#include <random>\n");
      si->random = 1;
    }
    if (!si->sfmlGraphics && (strstr(name, "sf::") == name)) {
    ...
    if (!si->astar && (strstr(name, "AStar::") == name)) {
        ...
        if (incfile->package) {
          if (!strcmp(incfile->package, nsname)) {
            ...
            if (strcmp(incfile->package, "sf") !=
                0) {  // Don't #include from sfml (handled by
                      // print_include_stdlib())
              print("#include \"%s/%s.%s\"\n", incfile->package, incfile->name,
                    file_ext);
            }
          }
        } else {
