...
//            if (animation == -1)
//                animation = 1;
//            sprite = sprites.getCharacterSprite(l, h, (*c)->getId(), (int)
//            (*c)->getDirection(), animation); sprite.setPosition(Vector2f(l *
//            ic, h * jc)); window.draw(sprite);
//        }
//    }
//}
  ...
  while (window.isOpen()) {
    ...
    for (auto c = chars.begin(); c != chars.end(); ++c) {
      ...
      if (ic >= xv && ic < xv + n && jc >= yv && jc < yv + m) {
        ...
        if (animation == -1)
          animation = 1;
        sprite = sprites.getCharacterSprite(
            l, h, (*c)->getId(), (int)(*c)->getDirection(), animation);
