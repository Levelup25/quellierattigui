...
void Render::display() {
    ...
    vector<Color> colors = {Color::White, Color::Blue, Color::Green, Color::Red, Color::Yellow};
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (event.mouseButton.button == sf::Mouse::Right) {
                    if (state->isFighting() &&
                            state->getMainTeam()->getCharacter(X, Y) != nullptr && state->getCharacter(X, Y)->getPv() > 0) {
        ...
        if (state->getCharacter(X, Y) != nullptr && state->getCharacter(X, Y)->getPv() > 0) {
