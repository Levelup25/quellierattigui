...
void Render::drawZones(RenderWindow& window, sf::View& view)
{
    ...
    if (state->isFighting() && state->getFight()->getTurn() % 2 == 1)
    {
        ...
        {
            AttackCommand* atkcmd = new AttackCommand(state, engine, selectedcharacter,{X, Y}, abilityNumber);
            atkcmd->setZones();
...
void Render::drawCharacters(RenderWindow& window, sf::View& view, Sprites& sprites, vector<Character*> chars)
{
    sf::Sprite sprite;
    RectangleShape zone(Vector2f(l, h));
    zone.setFillColor(Color::Transparent);
...
void Render::drawAnimations(RenderWindow& window, sf::View& view, Sprites& sprites)
{
    sf::Sprite sprite;
    window.setView(view);
            ...
            if (event.type == sf::Event::MouseButtonPressed)
            {
                ...
                else if (event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    else
                    {
                        ...
                        {
                            engine->addCommand(new AttackCommand(state, engine, selectedcharacter,{X, Y}, abilityNumber));
                            state->etatCombat = 0;
                        }
                    }
                }
