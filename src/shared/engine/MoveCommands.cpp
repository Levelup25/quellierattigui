...
vector<int> MoveCommands::getDiff()
{
    return
    {
        (int) i - (int) character->getI(), (int) j - (int) character->getJ()
    };
}
...
vector<vector<int>> MoveCommands::getPath()
{
    ...
    auto path =
            generator.findPath({(int) i - xv, (int) j - yv},
    {
                               i0 - xv, j0 - yv
    });
...
void MoveCommands::execute()
{
    ...
    else if (state->getCell(i, j)->getContent() <= 1 && !state->isFighting())
    {
        ...
        else if ((i + 1) % n == 0 && i + 1 < state->getI() && state->getCell(i + 1, j)->getContent() == 0)
        {
            engine->addCommand(new MoveCommand(state, character, i + 1, j));
        }
        else if (j % m == 0 && j > 0 && state->getCell(i, j - 1)->getContent() == 0)
        {
            engine->addCommand(new MoveCommand(state, character, i, j - 1));
        }
        else if ((j + 1) % m == 0 && j + 1 < state->getJ() && state->getCell(i, j + 1)->getContent() == 0)
        {
            engine->addCommand(new MoveCommand(state, character, i, j + 1));
        }
    }
